...
issue #14: 副露 (鳴き) を MeldsRecognizer が 4 家分認識する。
...
from recognition.melds_recognizer import MeldsRecognizer
...
_melds_recognizer: MeldsRecognizer | None = None
...
def _get_melds_recognizer() -> MeldsRecognizer:
    global _melds_recognizer
    if _melds_recognizer is None:
        _melds_recognizer = MeldsRecognizer(_TEMPLATE_DIR)
    return _melds_recognizer
...
def handle_frame(req: dict[str, Any]) -> dict[str, Any]:
    ...
    if bgr is not None:
        ...
        try:
            tenhou_json["melds"] = _get_melds_recognizer().recognize_melds(
                bgr, roi_calib.get("melds")
            )
        except Exception:  # noqa: BLE001 — recognition プロセスを落とさない
            logger.warning("melds recognition failed for id=%s", frame_id, exc_info=True)
